...
class Password : Activity() {
    ...
    var contactsList = ArrayList<PwEn>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val savedContacts = db!!.pwDao().getAll()
        if(savedContacts.isNotEmpty()){
            contactsList.addAll(savedContacts)
        }

        //어댑터, 아이템 클릭 : 아이템 삭제
        val adapter = PwListAdapter(contactsList)
        adapter.setItemClickListener(object : PwListAdapter.OnItemClickListener{
            override fun onClick(v: View, position: Int) {

                val contacts = contactsList[position]

                db?.pwDao()?.delete(contacts = contacts) //DB에서 삭제
        ...
        mPlusButton.setOnClickListener {
            ...
            builder.setView(dialogView)
                .setPositiveButton("확인") { dialogInterface, i ->
                    val contact = PwEn(0, dialogText.text.toString()) //Contacts 생성
                    db?.pwDao()?.insertAll(contact) //DB에 추가
