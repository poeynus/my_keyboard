...
class Email : Activity() {
    ...
    var contactsList = ArrayList<EmailEn>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val savedContacts = db!!.emailDao().getAll()
        if(savedContacts.isNotEmpty()){
            contactsList.addAll(savedContacts)
        }

        //어댑터, 아이템 클릭 : 아이템 삭제
        val adapter = EmaillistAdapter(contactsList)
        adapter.setItemClickListener(object : EmaillistAdapter.OnItemClickListener{
            override fun onClick(v: View, position: Int) {

                val contacts = contactsList[position]

                db?.emailDao()?.delete(contacts = contacts) //DB에서 삭제
        ...
        mPlusButton.setOnClickListener {
            ...
            builder.setView(dialogView)
                .setPositiveButton("확인") { dialogInterface, i ->
                    val contact = EmailEn(0, dialogText.text.toString()) //Contacts 생성
                    db?.emailDao()?.insertAll(contact) //DB에 추가
