...
class Address : Activity() {

    val TAG = "ListActivity"
    var db : AppDatabase? = null
    var contactsList = ArrayList<AddressEn>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val savedContacts = db!!.addressDao().getAll()
        if(savedContacts.isNotEmpty()){
            contactsList.addAll(savedContacts)
        }

        //어댑터, 아이템 클릭 : 아이템 삭제
        val adapter = AddressListAdapter(contactsList)
        adapter.setItemClickListener(object : AddressListAdapter.OnItemClickListener{
            override fun onClick(v: View, position: Int) {

                val contacts = contactsList[position]

                db?.addressDao()?.delete(contacts = contacts) //DB에서 삭제
        ...
        mPlusButton.setOnClickListener {
            ...
            builder.setView(dialogView)
                .setPositiveButton("확인") { dialogInterface, i ->
                    val contact = AddressEn(0, dialogText.text.toString()) //Contacts 생성
                    db?.addressDao()?.insertAll(contact) //DB에 추가
